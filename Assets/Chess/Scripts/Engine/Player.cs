...
namespace ChessLibrary
{
	...
	public class Player
	{
		public Type m_Type;		// Type of the player i.e. computer or human
		private Side m_Side;		// Player side i.e. white or black
		private string m_Name;		// Name of the Player
		private GameObject m_Image;		// Image of the player
        ...
        internal Player()
        {
            m_Side = PlayerSide;
            m_Type = PlayerType;
            m_MaxThinkTime = new TimeSpan(0, 0, 1);	// maximum think for 4 seconds
		...
		// Get the best move available to the player
		public Move GetBestMove(GameLevel level)
		{
			int alpha, beta;
			int depth;					// depth to which to do the search
			TimeSpan ElapsedTime= new TimeSpan(1);		// Total elpased time
            ...
            // Initialize constants
            int MIN_SCORE= -1000000;		// Minimum limit of negative for integer
			int MAX_SCORE= 1000000;		// Maximum limit of positive integer
            if (level == GameLevel.Intermediate)
            {
                MIN_SCORE = -10000000;      // Minimum limit of negative for integer
                MAX_SCORE = 10000000;		// Maximum limit of positive integer
            }
            else if (level == GameLevel.Master)
            {
                MIN_SCORE = -100000000;      // Minimum limit of negative for integer
                MAX_SCORE = 100000000;		// Maximum limit of positive integer
            }
			...
			int MoveCounter;
            //UnityEngine.Random RandGenerator = new UnityEngine.Random();
			...
			for (depth = 1;; depth++)	// Keep doing a depth search 
			{
				...
				// Check if the user time has expired
				ElapsedTime=DateTime.Now - ThinkStartTime;
				if ( ElapsedTime.Ticks > (m_MaxThinkTime.Ticks*0.25))	// More than 25 percent time is available
					break;							// Force break the loop
			}
