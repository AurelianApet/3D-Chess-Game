...
namespace ChessLibrary
{
    ...
    public class Rules
    {
		...
		public bool CheckForDrawwithTotalPieces()
		{
			if (m_Board.GetSideCell (Side.SideType.Black).Count < 3 && m_Board.GetSideCell (Side.SideType.White).Count < 3)
				return true;
			else
				return false;
		...
		public bool isThreeFoldRepetition() {
            object[] allMoves = new object[m_Game.GetAllMoves ().Count];

            allMoves = m_Game.GetAllMoves ().ToArray();
			int currentMoveIndex = 0;//allMoves.Length-1;
...
//			if (IsUnderCheck (m_Game.GameTurn)) {
//				UnityEngine.Debug.Log (" Under Check");
//			}
            if ((firstFoldMoveIndex >= 8) && allMoves.Length>8 && (IsUnderCheck(m_Game.GameTurn) ))//|| IsUnderCheck(Side.SideType.White))) 
			{
				
                string thirdFoldMoveMade = allMoves [(currentMoveIndex)].ToString();
                string secondFoldMoveMade = allMoves [(secondFoldMoveIndex)].ToString();
                string firstFoldMoveMade = allMoves [(firstFoldMoveIndex)].ToString();				
				if ((thirdFoldMoveMade.ToString ()).Equals (secondFoldMoveMade.ToString ()) &&
				    secondFoldMoveMade.ToString ().Equals (firstFoldMoveMade.ToString ()) /*&&
...
//						thirdFoldMoveMade = (Move)allMoves[(currentMoveIndex - gap)];
//						secondFoldMoveMade = (Move)allMoves[(secondFoldMoveIndex - gap)];
//						firstFoldMoveMade = (Move)allMoves[(firstFoldMoveIndex - gap)];
//
//						if(!thirdFoldMoveMade.ToString().Equals(secondFoldMoveMade.ToString()) || !secondFoldMoveMade.ToString().Equals(firstFoldMoveMade.ToString()))
//							break;
//					}
//					if(gap == distance)
//						return true;
...
//			}
        ...
        public bool CanMove(Move move)
        {
            ...
            if (!LegalMoves.Contains(m_Board[move.EndCell]))    // Not a legal move
                return false;  // Return illegal move error
		...
		private void SetMoveType(Move move)
		{
			...
			if (move.StartCell.piece != null && move.StartCell.piece.Type == Piece.PieceType.Pawn)
			{
				...
				if (move.EndCell.row == Board.MAX_ROW || move.EndCell.row == 1)
		...
		private void DoPromoMove(Move move)
		{
			...
			if (move.PromoPiece==null)
				m_Board[move.EndCell].piece = new Piece(Board.MAX_COL == 8 ? Piece.PieceType.Queen : Piece.PieceType.Knight, m_Board[move.EndCell].piece.Side, move.EndCell.piece.m_prevpos);	// Set the end cell to queen
			else
		...
		public void UndoMove(Move move)
		{
			...
			if (move.Type == Move.MoveType.TowerMove)
			{
				...
				if (move.EndCell.col > move.StartCell.col) // moving right
				{
					...
					Cell target = m_Board[move.StartCell.row, Board.MAX_COL];	// Get the rook orignal position
		...
		public ArrayList GenerateAllLegalMoves(Side PlayerSide)
		{
            ...
            int n = 0;
            for (int i = 1; i < TotalMoves.Count; i++)
            {
                Move m = (Move)TotalMoves[i];
                Move pv = (Move)TotalMoves[i - 1];
                if (m.Score != 0)
                    m.Score = m.Score;
                if (m.Score == pv.Score)
                {
                    //int r = random.Next() % (i - n + 1);
                    //r = n + r;
        ...
        public void SetCellPiece(Cell cell, Piece piece)
        {
            m_Board[cell].piece.Type = piece.Type;
        }
		...
		private void GetPawnMoves(Cell source, ArrayList moves)
		{
			...
			if (source.piece.Side.isWhite())
			{
				...
				if (newcell!=null && newcell.IsOwnedByEnemy(source) && !newcell.IsNotCaptureable(source)) // Top cell is available for the move
					moves.Add(newcell);
			...
			else
			{
				...
				if (newcell!=null && newcell.IsOwnedByEnemy(source) && !newcell.IsNotCaptureable(source)) // Bottom cell is available for the move
					moves.Add(newcell);
		...
		private void GetKnightMoves(Cell source, ArrayList moves)
		{
			...
			if (newcell!=null)
			{
				...
				if (newcell!=null && !newcell.IsOwned(source) && !newcell.IsNotCaptureable(source)) 
					moves.Add(newcell);
		...
		private void GetKingMoves(Cell source, ArrayList moves)
		{
			...
			newcell = m_Board.TopCell(source);
			if (newcell!=null && !newcell.IsOwned(source) && IsInVale(newcell)) // target cell is empty or is owned by the enemy piece or is in bound of palace
				moves.Add(newcell);
			...
			if (m_Board[source].piece.Moves == 0)
			{
				...
				newcell = m_Board.RightCell(source);
				if (newcell!=null && newcell.IsEmpty() && IsInVale(newcell)) // cell is empty or is in bound of palace
				{
					if (!CauseCheck(new Move(source, newcell))) // Inbetween cell is not under check
					{
						...
						if (newcell!=null && newcell.IsEmpty() && IsInVale(newcell)) // cell is empty or is in bound of palace
                        {
                            ...
                            newcell = m_Board.RightCell(newcell);
							if (newcell!=null && !newcell.IsEmpty() && newcell.piece.IsRook() && newcell.piece.Moves==0)	// check if the rook piece has not yet moved
        ...
        private bool IsInVale(Cell newcell)
        {
            if (Board.MAX_COL == 8)
                return true;
            return ((newcell.row == 1 || newcell.row == 2 || newcell.row == (Board.MAX_ROW - 1) || newcell.row == Board.MAX_ROW) && (newcell.col >= 4 && newcell.col <= 6));
        }
