...
namespace ChessLibrary
{
    ...
    public class Game
    {
        ...
        // Event used to fire computer thinking status

        public Board Board;
        // expose the game board to outside world
        public Side.SideType GameTurn;
        // Current game turn i.e. White or Black

        private Stack m_MovesHistory;
        // Contains all moves made by the user
        private Stack m_RedoMovesHistory;
        // Contains all the Redo moves made by the user
        private Rules m_Rules;
        // Contains all the chess rules
        private Player m_WhitePlayer = null;
        // White Player objectg
        private Player m_BlackPlayer = null;
        // Black player object

        public bool DoNullMovePruning;
        // True when compute should do null move pruning to speed up search
        public bool DoPrincipleVariation;
        // True when computer should use principle variation to optimize search
        public bool DoQuiescentSearch = true;
        // Return true when computer should do Queiscent search
        ...
        public void Reset(int mode, int formationMode)
        {
            ...
            Board.Init(mode, formationMode);	// Initialize the board object
        }
        ...
        // Return if can move
        public bool CanMove(string source, string dest)
        {
            bool bCan = false;
            ...
            {
                Move UserMove = new Move(this.Board[source], this.Board[dest]); // create the move object
                bCan = m_Rules.CanMove(UserMove);
            }
            else
                bCan = false;
            return bCan;	// Executed
        }
		...
		public bool CheckForDrawCon1()
		{
			return m_Rules.CheckForDrawwithTotalPieces();
		}

		public bool CheckForThreeFoldRepition()
		{
			return m_Rules.isThreeFoldRepetition();
		}
        ...
        public void SetPromoPiece(Piece PromoPiece)
        {
            ...
            if (m_MovesHistory.Count > 0)
            {
                Move move = (Move)m_MovesHistory.Peek();	// Get the user move from his moves history
                move.EndCell.piece = PromoPiece;	// Set the promo piece
                move.PromoPiece = PromoPiece;		// Update the promo piece variable
                m_Rules.SetCellPiece(move.EndCell, PromoPiece);
            }
        }

        /// <summary>
        /// Photon Region
        /// Runs After PUN RPC call and UPDATES the Moves
        /// </summary>

        //		public void Set_Moves(Move new_move)
        //		{
        //			Move move = new_move;
        //		}
        //
        //		public void Get_Moves(Move old_move)
        //		{
        //			return old_move;
        //		}
    }
